...
const Demo = () => {
    ...
    //to display articles even after refresh
    useEffect(() => {
    ...
    const handelSubmit = async (e) => {
        ...
        if (data?.summary) {
            ...
            // Update local storage with out loaded article summaries
            ...
            <div className="flex flex-col gap-1 max-h-60 overflow-y-auto ">
                {allArticles.map((item,index)=>(
                    <div key={`link-${index}`} onClick={()=>setArticle(item)} className="link_card">
                        ...
                        <p className="flex-1 font-satoshi text-blue font-medium text-sm truncate">{item.url}</p>
                    </div>
